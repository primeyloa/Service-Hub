...
The program connects to the SQLite database at `db/service_hub.db` (a `db`
folder is created inside the `University-Campus-Service-Hub` folder when you
first run it) and creates the tables if they do not exist. It then seeds the
database from the CSV dataset in the repo-level `data` folder
(`../data/locations_template.csv`, `roads_template.csv`, `resources_template.csv`
and `service_requests_template.csv`).
...
The program creates the `db` folder and `reports` folder itself, so a missing
folder should not be the cause. Re-run with the full stack trace visible
(`mvn exec:java -Dexec.args="--cli"`) and share the error with the team.
